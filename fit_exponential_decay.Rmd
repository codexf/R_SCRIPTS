...
# Fit data with a one-phase exponential model  
$$Y = span \cdot e^{-k\cdot t} + plateau$$
...
## Use `nls` by guessing the single starting value  
...
# Fit data with a two-phase exponential model
$$Y = span\_fast \cdot e^{-k\_fast \cdot t} + span\_slow \cdot e^{-k\_slow \cdot t} + plateau$$
...
If you run this step multiple times, you will notice that the order of the term with alpha and beta and the term with gamma and delta can be interchanged because of the ramdom starting parameters. You may want to set the term with a larger decay constant as span_fast and k_fast
...
## Comparing one-phase and two-phase exponential models
...
ggplot() +
  ...
  geom_line(aes(t, .fitted, colour="one-phase"), preds_1)+
  geom_line(aes(t, .fitted, colour="two-phase"), preds_2)+
  scale_color_manual(name = "", values = c("one-phase" = "darkblue", "two-phase" = "red"))
